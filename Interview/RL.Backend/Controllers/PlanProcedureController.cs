...
public class PlanProcedureController : ControllerBase
{
    ...
    [HttpPost("AddUser")]
    public async Task<IActionResult> Post(AddUserToPlanProcedureCommand command, CancellationToken token)
    {
    ...
    public async Task<IActionResult> Delete(RemoveUserFromPlanProcedureCommand command, CancellationToken token)
    {
        var response = await _mediator.Send(command, token);
        if (response.Succeeded)
        {
